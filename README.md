...
But these types do have some built-in properties. Every string value has a number 
of methods. The most useful ones are probably slice and indexOf, which resemble the 
...
// ? 5
```
One difference is that a string's indexOf can take a string containing more than 
